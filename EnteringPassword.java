...
// Класс EnteringPassword предоставляет графический интерфейс для ввода пароля в серверном приложении
// Основные функции:
// - Создание окна с полем для ввода пароля, кнопкой для показа/скрытия пароля и кнопками "Вход" и "Вернуться в главное меню"
// - Проверка введённого пароля с использованием Argon2 для аутентификации
// - Создание конфигурационного файла при первом входе и шифрование параметров с использованием Safety
// - Переход к управлению сервером (ServerControl) при успешной аутентификации
// - Поддержка тёмной темы интерфейса с использованием Swing

public class EnteringPassword {

    private static final String CONFIG_FILE_NAME = "config_user.properties"; // Имя файла конфигурации для регистрации
    private static final String CONFIG_FILE_NAME_SERVER = "config.properties"; // Имя основного файла конфигурации сервера
    private static Config config = new Config(); // Объект для работы с конфигурационными файлами
    private static JFrame frame; // Главное окно приложения
    private static Encryption encryption = new Encryption(); // Объект для операций шифрования (Argon2)
    private static JPanel textPanel; // Панель для поля пароля и кнопки "Показать"
    private static Argon2 argon2 = Argon2Factory.create(); // Объект Argon2 для проверки пароля
    private static StartServer startServer = new StartServer(); // Объект для возврата в главное меню
